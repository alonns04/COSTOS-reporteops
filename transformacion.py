...
def transformar_df(df):
    ...
    if "Tipo" not in df.columns:
        if "TipoLancamento" in df.columns:
            df = df.rename(columns={"TipoLancamento": "Tipo"})
        elif "TipoLanzamiento" in df.columns:
            df = df.rename(columns={"TipoLanzamiento": "Tipo"})
        elif "TipoProduto" in df.columns:
            df = df.rename(columns={"TipoProduto": "Tipo"})
    ...
    unidad_fisica_col = "UnidadeFisica"
    if unidad_fisica_col not in df.columns:
        unidad_fisica_col = "UnidadFisica"
    ...
    # ---------------------------------
    # Priorizar cantidades correctas en
    # mayúsculas usando valores en minúsculas
    # ---------------------------------

    for cantidad_minuscula, cantidad_mayuscula in [
        ("cantidad prevista", "Cantidad Prevista"),
        ("cantidad real", "Cantidad Real")
    ]:
        if cantidad_minuscula in df_final.columns:
            df_final[cantidad_mayuscula] = df_final[cantidad_minuscula].where(
                pd.notna(df_final[cantidad_minuscula]),
                df_final.get(cantidad_mayuscula, np.nan)
            )

    # ---------------------------------
    # Eliminar columnas minúsculas duplicadas
    # ---------------------------------
    ...
    columnas_minusculas_existentes = [
        col
        for col in columnas_minusculas
        if col in df_final.columns
    ]

    if columnas_minusculas_existentes:
        df_final = df_final.drop(
            columns=columnas_minusculas_existentes
        )
